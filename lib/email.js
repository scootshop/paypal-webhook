...
// Logo real: https://scootshop.co/img/0-removebg-preview.png
...
// =========================
// Brand config (centralizado)
// =========================
const BRAND = Object.freeze({
  NAME: "SCOOT SHOP",
  URL: "https://scootshop.co/",
  LOGO_URL: "https://scootshop.co/img/0-removebg-preview.png", // ✅ ruta real
  SUPPORT_EMAIL: "support@scootshop.co",
  WHATSAPP_PHONE: "34666318747",
  DEFAULT_PRODUCT_IMG: "https://scootshop.co/patinetes/series-ix/ix3/img/1.jpg",
});
...
function renderOrderEmailHTML({
  ...
  const safeAmount =
    amountValue && currencyCode
      ? escapeHtml(formatMoney(amountValue, currencyCode))
      : "";

  const safeProductImg = escapeHtml(
    productImageUrl || BRAND.DEFAULT_PRODUCT_IMG
  );

  const primaryCtaUrl = escapeHtml(orderUrl || BRAND.URL);

  const whatsappText = `Hola! Tengo una consulta sobre mi pedido #${orderId || "—"}`;
  const WHATSAPP_URL =
    `https://wa.me/${BRAND.WHATSAPP_PHONE}?text=` +
    ...
    <tr>
      <td align="center" style="padding:0 12px;">
        <table role="presentation" width="640" cellspacing="0" cellpadding="0" border="0" style="max-width:640px;width:100%;">
          ...
          <!-- Top gradient bar -->
                   <tr>
            <td style="background:linear-gradient(40deg,#dedede,#ffffff,#ffffff,#ffffff);border-radius:18px 18px 0 0;padding:18px 18px 12px 18px;text-align:center;">
              <a href="${BRAND.URL}" target="_blank" rel="noopener noreferrer" style="text-decoration:none;display:inline-block;">
                <img src="${BRAND.LOGO_URL}" width="140" alt="${BRAND.NAME}" style="display:block;border:0;outline:none;height:auto;margin:0 auto;">
              </a>
            </td>
          </tr>
          ...
          <tr>
            <td style="background:#FFFFFF;border-radius:0 0 18px 18px;box-shadow:0 10px 30px rgba(16,24,40,.10);padding:22px 18px 18px 18px;">
              ...
              <div style="margin-top:14px;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.7;color:#6B7280;">
                ...
                <a href="mailto:${BRAND.SUPPORT_EMAIL}" style="color:#111827;text-decoration:underline;">${BRAND.SUPPORT_EMAIL}</a>.
              </div>
...
Soporte: ${BRAND.SUPPORT_EMAIL}`;
  ...
  if (bccClean) payload.bcc = [bccClean];
...
// =========================
// Generic Resend relay (for non-payment events)
// Sends custom HTML/text provided by the caller
// =========================
async function sendRawEmailResend({ to, bcc, subject, html, text }) {
  const payload = {
  ...
  const out = await r.text();
  console.log("resend-raw:", r.status, out.slice(0, 300));
  if (!r.ok) throw new Error(`resend raw send error: ${r.status}`);
}
